...
% do our thing first for individual methods and then across methods? easier for cosmicflows
...
\sigma_D^2=\sigma_s^2+\sigma_r^2\ .
	\label{eq:bayq}
\end{equation}
If the systematic error is a scale factor error as Fig.~\ref{fig:hqp} suggests, $\sigma_r=sD$ where the scale factor $s$ and the random error $\sigma_r$ are independent of the distance. Here we ignore the possibility of having a zero setting error in the systematic component for numerical reasons explained in Section~\ref{sec:lqm}. % add the goddamn prior here!

We now use \texttt{emcee} to sample the posterior over the parameter set $\pmb{\theta}=(f,s,\sigma_r)$ using 100 walkers and 10000 steps. According to the discrepancy plot in Fig.~\ref{fig:discq}, this model is able to replicate method H errors for the 29 galaxies with $N>25$ measurements. The corner plot showing the posterior sampling made by \texttt{emcee} is shown in Fig.~\ref{fig:cornerq}, which shows that the random error component is $\sigma_r=1.8_{-1.0}^{+0.8}$ Mpc, the scale factor is $s^2=0.07_{-0.01}^{+0.01}$, and the error variance scale factor is $f=0.50_{-0.07}^{+0.10}$. In particular, the error variance scale factor is in agreement with Fig.~\ref{fig:ee}. The working distance range and overall fitting of this model is shown in Fig.~\ref{fig:drawsq}, where the method H errors corresponding to galaxies with more than 25 TF distance measurements are plotted along the expected values $e=\{e_j(\pmb{\theta}_k)\}$ for parameter sets $\pmb{\theta}_k$ drawn from the posterior probability distribution. 
\begin{figure*}
	\includegraphics[scale=0.7]{cornerq}
    \caption{Corner plot showing the \texttt{emcee} sampling of the posterior probability distribution (equation \ref{eq:ppd}) for the quadrature Bayesian model parameters $\pmb{\theta}=(s,\sigma_r,f)$ based on errors estimated using method H for galaxies with more than 25 TF distance measurements. The dashed lines indicate the 16th, 50th, and 84th percentile of the marginalized distribution of each parameter (shown at the top of each column), and the blue solid lines indicate the mean. This plot was made using the \texttt{corner} Python module .}
    \label{fig:cornerq}
\end{figure*}
\begin{figure}
...
\begin{figure}
    ...
    \label{fig:drawsq}
\end{figure}
The discrepancy plot for method M errors (Fig.~\ref{fig:discq2}) looks very similar to Fig.~\ref{fig:discq} which means that the Bayesian quadrature model also replicates method M errors for galaxies with more than 25 measurements. Fig.~\ref{fig:cornerq2}, shows that $\sigma_r=1.8_{-1.0}^{+0.8}$ Mpc, $s^2=0.07_{-0.01}^{+0.01}$, and $f=0.50_{-0.07}^{+0.10}$. %change the previous values and talk about how its similar to fig 4 and about how both show a linear trend but here it is more pronounced because sigma_r is tiny af because it is so obviously , so thats why we try our luck with a linear model... which later then shows that there is no random component but a zero offset systematic error yay. both models are equivalent but for numerical reasons we reject the complete coz it predicts  (at least one) negative values for the distance always? i need to write it in this weird way because i only know that the model kind of works after n>15 thanks to doing them separately. full model shows numerical instability due to the degeneracy between the zero offset error and the random error.
\begin{figure*}
	\includegraphics[scale=0.7]{cornerq2}
    \caption{Corner plot showing the \texttt{emcee} sampling of the posterior probability distribution (equation \ref{eq:ppd}) for the quadrature Bayesian model parameters $\pmb{\theta}=(s,\sigma_r,f)$ based on errors estimated using method M for galaxies with more than 25 TF distance measurements. The dashed lines indicate the 16th, 50th, and 84th percentile of the marginalized distribution of each parameter (shown at the top of each column), and the blue solid lines indicate the mean. This plot was made using the \texttt{corner} Python module .}
    \label{fig:cornerq}
\end{figure*}
\begin{figure}
...
\begin{figure}
    ...
    \caption{Projection of parameter set samples from the posterior probability distribution of the Bayesian quadrature model onto the $D_G$ vs. $\sigma_D$ scatter plot for errors estimated using method M for galaxies with more than 25 TF distance measurements.}
    \label{fig:drawsq}
\end{figure}
...
Evidently, the sample of galaxies for which this model works is very small, which means that
...
\label{sec:blm} % used for referring to this section from elsewhere
