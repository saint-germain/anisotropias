...
Efforts to reduce the uncertainty in the estimate the Hubble constant are single-method \citet{hubsn2018}.
...
Bayesian analysis of systematic uncertainties for cosmology when using SNIa-derived galactic parameters \citet{unity}

\citet{6df} attempt to predict redshift-derived distance errors within a Bayesian framework yields 26\%.

Exploration of prior discrepancy modeling in model estimation \citet{priordesc}

Importance of distance and catalogs \citet{catetg,catspi} in:



The NASA/IPAC Extragalactic Distance (NED) catalog of Redshift-Independent Distances \citet{ned} is  a catalog with the following properties

Determining whether a galaxy belongs to a group by analyzing their common properties \citet{gg3500} 
Determining the spatial distribution of galaxies in order to study large-scale structure \citet{gallargescale}

Studies of anisotropy be it of morphological types \citet{morphanis}

\citet{cosmicflows} distance errors are weighted standard deviations
